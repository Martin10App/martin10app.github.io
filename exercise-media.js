...
const EXERCISE_MEDIA_CATALOG='exercise-media.json?v=1';
...
async function loadExerciseMediaCatalog(){
  ...
  _exerciseMediaPromise=(async()=>{
    ...
    try{
      const url=location.protocol==='file:'?'https://martin10app.github.io/exercise-media.json?v=1':EXERCISE_MEDIA_CATALOG;
...
async function loadExerciseMovement(ex,slotId){
  ...
  try{
    ...
    slot.innerHTML=`<div class="ex-media-card">
      <div class="ex-media-frame"><img src="${esc(initial)}" alt="Demostraci\u00f3n de ${esc(ex.name)}" width="300" height="300" decoding="async" data-gif="${esc(gif)}" data-still="${esc(still)}" data-playing="${reduce?'0':'1'}"></div>
    ...
    if(image)image.addEventListener('error',()=>renderExerciseMediaError(slot,'No se pudo cargar la animacion'));
...
function toggleExerciseMovement(button){
  ...
  image.dataset.playing=playing?'0':'1';image.src=playing?image.dataset.still:image.dataset.gif;
