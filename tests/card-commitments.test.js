...
const FUNCTIONS=['money','sumMoney','parseMoney','fmt','normalizeGymLookup','parseFinanceMonth','categoryTokens','commitmentCategories','shownCommitmentAmount',
...
// Categorías comunes de tarjeta con monto mensual, como las usa Martín, más dos tarjetas de cards1:
// una sin compras (no debe elegirse nunca) y otra con compras en cuotas.
...
test('[G1] una tarjeta que no existe o dos que coinciden: error claro y nada anotado',()=>{
  const ctx=makeContext();
  assert.match(say(ctx,'anotá 800 en la tarjeta itau').error,/No encontré la tarjeta "itau"\. Las que tenés: OCA card, Banco card, Banco Recompensa, UTE, Master/);
...
test('[G1] las card_ sin compras nunca se eligen; con compras, sumar agrega una compra y no congela el mes',()=>{
  const ctx=makeContext();
  assert.equal(say(ctx,'anotame 900 en la visa'),null,'sin tarjeta que coincida no se intercepta');
  assert.match(ctx.ejecutarAccion({op:'ajustar_monto',cat:'Visa',delta:900,m:9,y:2026}),/^No encontr|categor/i);
  assert.equal(ctx.db.pf4.card_v_9_2026,undefined);
